...
namespace BusinessLogic.Services
{
	internal class MovieService : IMovieService
	{
		...
		public async Task AddFeedbackAsync(FeedbackCreationModel model)
		{
			if (model == null) throw new HttpException(Errors.InvalidRequestData, HttpStatusCode.BadRequest);
			var feedback = mapper.Map<Feedback>(model);
			await feedbacks.InsertAsync(feedback);
			await feedbacks.SaveAsync();
		}

		public async Task DeleteFeedbackAsync(int feedbackId)
		{
			if (feedbackId < 0) throw new HttpException(Errors.NegativeId, HttpStatusCode.BadRequest);
			var feedback = await feedbacks.GetItemBySpec(new FeedbacsSpecs.GetById(feedbackId))
										  ?? throw new HttpException(Errors.NotFoundById, HttpStatusCode.NotFound);
			feedbacks.Delete(feedback);
			await feedbacks.SaveAsync();
		}

		public async Task ApproveFeedbackAsync(int feedbackId)
		{
		...
		public async Task<PaginationResultModel<MovieDto>> GetMoviesWithNotApprovedFeedbacksAsync(int pageIndex, int pageSize) 
		{
			var result=  mapper.Map<IEnumerable<MovieDto>>(await movies.GetListBySpec(new MovieSpecs.GetWithNotApprovedFeedbacks()));
			return new PaginationResultModel<MovieDto>(result, pageIndex, pageSize);
		}
